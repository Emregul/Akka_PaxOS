...
case class catchupLogPositionResponse(val logPosition:Int, val filledValue:String, val isFilled:Boolean) extends CatchupMessage
...
class ReplicaManagerApplication(val n: Int, val numberOfReplicas : Int) extends Bootable {
  //#setup
  val system = ActorSystem("ReplicaManager", ConfigFactory.load.getConfig("serverlocal1"))
...
class Replica(val masterPath:String,ID:Int = -1, val numberOfReplicas:Int = -1) extends Actor{
   ...
   def active(actor: ActorRef): Actor.Receive = {
    ...
    case catchupLogPosition(logPosition:Int) => if(isReplicaActive){
      if(logPosition < nextLogPosition) {
        sender ! new catchupLogPositionResponse(logPosition, blogPosts(logPosition), true)
        }else{
         sender ! new catchupLogPositionResponse(logPosition, "", false) 
        }
      }
    case catchupLogPositionResponse(logPosition:Int, filledValue:String, isFilled:Boolean) => if(isReplicaActive){
      if(isFilled){
  ...
  def getNextAvailableBallotNumber():Int = {
	  if(lastSelectedBallotNumber >= lastReceivedBallotNumber){
	    ...
	    return (divident+1) * numberOfReplicas + (ID - 1)
	  }
  }
