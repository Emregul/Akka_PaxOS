...
class ReplicaManagerApplicationPromotion(val n: Int, val numberOfReplicas : Int) extends Bootable {
  ...
  val remotePath = "akka.tcp://MasterApplication@"+ ReplicaManagerPromotion.masterAddress.getHostName() + ":"+ ReplicaManagerPromotion.masterAddress.getPort() + "/user/master"
  val actor = system.actorOf(Props(classOf[ReplicaPromotion], remotePath,n, numberOfReplicas), "replica")
...
class ReplicaPromotion(val masterPath:String,ID:Int = -1, val numberOfReplicas:Int = -1) extends Actor{
  import context._
...
object ReplicaManagerPromotion {
  ...
  def main(args: Array[String]) {
    ...
    if (args(1).equals("local")) {
      ...
      //id = readInt
      for (i <- 1 to numberOfReplicas){ var multipleApps = new ReplicaManagerApplicationPromotion(i,numberOfReplicas) }
    }else{
      //reverse lookup id from current config ip
      val app = new ReplicaManagerApplicationPromotion(id,numberOfReplicas)
    }
